...
from constraints_file import generate_shift_employee_combinations, add_exactly_one_employee_per_shift_constraint, \
    ...
    add_maximize_employees_shift_preferences
...
def test_employees_get_their_shift_preferences():
    main_shift_start_time = datetime.datetime(2023, 12, 12, 9, 0)
    shift_duration = datetime.timedelta(hours=4)
    ...
    senior_employee1 = Employee("sen_emp1", EmployeePriorityEnum.LOW, EmployeeStatusEnum.senior_employee, employee_id="sen_emp1", preferences=[support_shift_1])
    senior_employee2 = Employee("sen_emp2", EmployeePriorityEnum.LOW, EmployeeStatusEnum.senior_employee, employee_id="sen_emp2", preferences=[main_shift])

    shifts = [main_shift, support_shift_1]
    employees = [senior_employee1, senior_employee2]
    ...
    add_exactly_one_employee_per_shift_constraint(shifts, employees, model, all_shifts)
    add_prevent_overlapping_shifts_for_employees_constraint(shifts, employees, model, all_shifts)
    add_maximize_employees_shift_preferences(employees, model, all_shifts)

    solver = cp_model.CpSolver()
    status = solver.Solve(model)

    assert (status == cp_model.OPTIMAL)
    senior_employee1_main_shift = all_shifts[ShiftCombinationsKey(senior_employee1.employee_id, support_shift_1.shift_id)]
    assert solver.Value(senior_employee1_main_shift) == True
    senior_employee2_support_shift_1 = all_shifts[ShiftCombinationsKey(senior_employee2.employee_id, main_shift.shift_id)]
    assert solver.Value(senior_employee2_support_shift_1) == True


def test_senior_employee_gets_his_prefs_and_another_senior_who_does_not_get_shifts():
    shift_a_start_time = datetime.datetime(2023, 12, 12, 9, 0)
    shift_duration = datetime.timedelta(hours=4)
    ...
    |a|b|c|
    """
    shift_a = Shift("shift_a", shift_type=ShiftTypesEnum.EVENING, start_time=shift_a_start_time, end_time=shift_a_start_time + shift_duration)
    shift_b = Shift("shift_b", shift_type=ShiftTypesEnum.EVENING, start_time=shift_a.end_time, end_time=shift_a.end_time + shift_duration)
    shift_c = Shift("shift_c", shift_type=ShiftTypesEnum.EVENING, start_time=shift_b.end_time, end_time=shift_b.end_time + shift_duration)

    senior_employee1 = Employee("sen_emp1", EmployeePriorityEnum.LOW, EmployeeStatusEnum.senior_employee, employee_id="sen_emp1", preferences=[shift_a, shift_b,shift_c])
    senior_employee2 = Employee("sen_emp2", EmployeePriorityEnum.LOW, EmployeeStatusEnum.senior_employee, employee_id="sen_emp2", preferences=[])
    ...
    assert (status == cp_model.OPTIMAL)
    senior_employee1_shift_a = all_shifts[ShiftCombinationsKey(senior_employee1.employee_id, shift_a.shift_id)]
    assert solver.Value(senior_employee1_shift_a) == True
    senior_employee1_shift_b = all_shifts[ShiftCombinationsKey(senior_employee1.employee_id, shift_b.shift_id)]
    assert solver.Value(senior_employee1_shift_b) == True
    senior_employee1_shift_c = all_shifts[ShiftCombinationsKey(senior_employee1.employee_id, shift_c.shift_id)]
    assert solver.Value(senior_employee1_shift_c) == True

    senior_employee2_shift_a = all_shifts[ShiftCombinationsKey(senior_employee2.employee_id, shift_a.shift_id)]
    assert solver.Value(senior_employee2_shift_a) == False
    senior_employee2_shift_b = all_shifts[ShiftCombinationsKey(senior_employee2.employee_id, shift_b.shift_id)]
    assert solver.Value(senior_employee2_shift_b) == False
    senior_employee2_shift_c = all_shifts[ShiftCombinationsKey(senior_employee2.employee_id, shift_c.shift_id)]
    assert solver.Value(senior_employee2_shift_c) == False


def test_senior_employee_gets_a_shift_even_though_a_new_employee_wants_that_shift():
    shift_a_start_time = datetime.datetime(2023, 12, 12, 9, 0)
    shift_duration = datetime.timedelta(hours=4)
    ...
    senior_employee1 = Employee("sen_emp1", EmployeePriorityEnum.HIGHEST, EmployeeStatusEnum.senior_employee, employee_id="sen_emp1", preferences=[])
    new_employee1 = Employee("new_emp1", EmployeePriorityEnum.LOW, EmployeeStatusEnum.new_employee, employee_id="new_emp1", preferences=[shift_a])

    shifts = [shift_a, shift_b]
    employees = [senior_employee1, new_employee1]
    ...
    new_employee1_shift_a = all_shifts[ShiftCombinationsKey(new_employee1.employee_id, shift_a.shift_id)]
    assert solver.Value(new_employee1_shift_a) == False
    new_employee1_shift_b = all_shifts[ShiftCombinationsKey(new_employee1.employee_id, shift_b.shift_id)]
    assert solver.Value(new_employee1_shift_b) == False


def test_2_senior_employees_wants_a_shift_but_the_one_with_higher_priority_got_it():
    shift_a_start_time = datetime.datetime(2023, 12, 12, 9, 0)
    shift_duration = datetime.timedelta(hours=4)
    ...
    """
    shift_a = Shift("shift_a", shift_type=ShiftTypesEnum.EVENING, start_time=shift_a_start_time, end_time=shift_a_start_time + shift_duration)
    shift_b = Shift("shift_b", shift_type=ShiftTypesEnum.EVENING, start_time=shift_a.start_time, end_time=shift_a.start_time + shift_duration)
    shift_c = Shift("shift_c", shift_type=ShiftTypesEnum.EVENING, start_time=shift_b.end_time, end_time=shift_b.end_time + shift_duration)

    senior_employee1 = Employee("sen_emp1", EmployeePriorityEnum.HIGHEST, EmployeeStatusEnum.senior_employee, employee_id="sen_emp1", preferences=[shift_a, shift_c])
    senior_employee2 = Employee("sen_emp2", EmployeePriorityEnum.HIGHEST, EmployeeStatusEnum.senior_employee, employee_id="sen_emp2", preferences=[shift_b])
    senior_employee3 = Employee("sen_emp3", EmployeePriorityEnum.HIGH, EmployeeStatusEnum.senior_employee, employee_id="sen_emp3", preferences=[shift_b])

    shifts = [shift_a, shift_b, shift_c]
    employees = [senior_employee1, senior_employee2, senior_employee3]
    ...
    senior_employee2_shift_b = all_shifts[ShiftCombinationsKey(senior_employee2.employee_id, shift_b.shift_id)]
    assert solver.Value(senior_employee2_shift_b) == True
    senior_employee3_shift_b = all_shifts[ShiftCombinationsKey(senior_employee3.employee_id, shift_b.shift_id)]
    assert solver.Value(senior_employee3_shift_b) == False
